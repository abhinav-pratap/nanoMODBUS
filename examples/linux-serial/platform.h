...
typedef struct {
    const char* rts_gpiochip;
    unsigned int rts_gpio_line_offset;
    bool rts_active_receive;
} rts_config_t;
...
typedef struct {
    ...
    const rts_config_t* rts_params;
} serial_conn_t;

int open_serial_conn(const char* portname, speed_t baudrate, const rts_config_t* rts, serial_conn_t* out_conn) {
    ...
    chip = gpiod_chip_open_by_name(rts->rts_gpiochip);
    if (!chip) {
    ...
    line = gpiod_chip_get_line(chip, rts->rts_gpio_line_offset);
    if (!line) {
    ...
    // 3. Request the line as output and set initial value to receive
    // "uart_rts" is a label visible in tools like gpioinfo
    ret = gpiod_line_request_output(line, "uart_rts", rts->rts_active_receive ? 0 : 1);
    if (ret < 0) {
...
int set_rts_receive(void* arg) {
    ...
    if (conn->rts_params == NULL) {
        // no rts
        return 0;
    }

    if (conn->rts_params->rts_active_receive) {
        // active receive
...
int32_t write_serial(const uint8_t* buf, uint16_t count, int32_t byte_timeout_ms, void* arg) {
    ...
    tty_delay(DV((count + 1), 11,
                 115200));    // wait until all data is transmitted, delay calculation borrowed from mdusd
